...
import test.common as aas_common
...
class _SetterForListOfPrimitives:
    ...
    def set_strings_from_jsonable(self, jsonable: Jsonable) -> None:
        """
        ...
        for i, jsonable_item in enumerate(array_like):
            try:
                item = _str_from_jsonable(jsonable_item)
    ...
    def set_integers_from_jsonable(self, jsonable: Jsonable) -> None:
        """
        ...
        for i, jsonable_item in enumerate(array_like):
            try:
                item = _int_from_jsonable(jsonable_item)
            except DeserializationException as exception:
                exception.path._prepend(IndexSegment(array_like, i))
                raise
    ...
    def set_booleans_from_jsonable(self, jsonable: Jsonable) -> None:
        """
        ...
        for i, jsonable_item in enumerate(array_like):
            try:
                item = _bool_from_jsonable(jsonable_item)
            except DeserializationException as exception:
                exception.path._prepend(IndexSegment(array_like, i))
                raise
...
def list_of_primitives_from_jsonable(jsonable: Jsonable) -> aas_types.ListOfPrimitives:
    """
    Parse an instance of :py:class:`.types.ListOfPrimitives` from the JSON-able
    structure :paramref:`jsonable`.

    :param jsonable: structure to be parsed
    :return: Parsed instance of :py:class:`.types.ListOfPrimitives`
    :raise: :py:class:`DeserializationException` if unexpected :paramref:`jsonable`
    """
    ...
    setter = _SetterForListOfPrimitives()

    for key, jsonable_value in jsonable.items():
        setter_method = _SETTER_MAP_FOR_LIST_OF_PRIMITIVES.get(key)
    ...
    return aas_types.ListOfPrimitives(setter.strings, setter.integers, setter.booleans)


_SETTER_MAP_FOR_LIST_OF_PRIMITIVES: Mapping[
    str, Callable[[_SetterForListOfPrimitives, Jsonable], None]
] = {
    "strings": _SetterForListOfPrimitives.set_strings_from_jsonable,
    "integers": _SetterForListOfPrimitives.set_integers_from_jsonable,
    "booleans": _SetterForListOfPrimitives.set_booleans_from_jsonable,
    "modelType": _SetterForListOfPrimitives.ignore,
}
...
class _Serializer(aas_types.AbstractTransformer[MutableJsonable]):
    ...
    def transform_list_of_primitives(
        ...
        jsonable["strings"] = [item for item in that.strings]

        jsonable["integers"] = [item for item in that.integers]

        jsonable["booleans"] = [item for item in that.booleans]
