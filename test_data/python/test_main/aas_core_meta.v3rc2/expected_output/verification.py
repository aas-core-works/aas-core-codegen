...
_REGEX_MATCHES_GLOBAL_ASSET_ID_LITERALLY = _construct_matches_global_asset_id_literally()
...
class _Transformer(
    ...
    def transform_data_specification_physical_unit(
        ...
        if not (
            lang_strings_have_unique_languages(that.definition)
        ):
            yield Error(
                'Definition specifies no duplicate languages'
            )
...
def verify_date_time_stamp_utc(
        that: str
) -> Iterator[Error]:
    """Verify the constraints of :paramref:`that`."""
