...
function constructMatchesXsDayTimeDuration(): RegExp {
  const dayTimeDurationRep = "-?P((([0-9]+D)(T(([0-9]+H)([0-9]+M)?([0-9]+(\\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\\.[0-9]+)?S)?|([0-9]+(\\.[0-9]+)?S)))?)|(T(([0-9]+H)([0-9]+M)?([0-9]+(\\.[0-9]+)?S)?|([0-9]+M)([0-9]+(\\.[0-9]+)?S)?|([0-9]+(\\.[0-9]+)?S))))";
...
const REGEXP_MATCHES_GLOBAL_ASSET_ID_LITERALLY = constructMatchesGlobalAssetIdLiterally();
...
class Verifier
  ...
  *transformReferenceWithContext(
    ...
    if (!(
      !(
        (
          that.type == AasTypes.ReferenceTypes.GlobalReference
          && that.keys.length >= 1
        )
      )
