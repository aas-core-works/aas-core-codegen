...
class EnhancedSomething
 ...
 public:
  ...
  const std::vector<types::Result>& some_results() const override {
    return instance_->some_results();
  }

  std::vector<types::Result>& mutable_some_results() override {
    return instance_->mutable_some_results();
  }

  void set_some_results(
    std::vector<types::Result> value
  ) override {
    instance_->set_some_results(value);
  }
