...
TEST_CASE("Test Descent over an SomeItem") {
...
TEST_CASE("Test DescentOnce over an SomeItem") {
  const std::shared_ptr<
    aas::types::ISomeItem
  > instance(
    test::common::examples::LoadMaxSomeItem()
  );
...
TEST_CASE("Test Descent over an AnotherItem") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an AnotherItem") {
  const std::shared_ptr<
    aas::types::IAnotherItem
  > instance(
    test::common::examples::LoadMaxAnotherItem()
  );

  const std::filesystem::path trace_path(
    DetermineDescentOnceDir()
      / "MaxAnotherItem.trace"
  );

  AssertOrRerecordDescentOnce(instance, trace_path);
}

TEST_CASE("Test Descent over an Simple") {
  const std::deque<std::filesystem::path> paths(
    test::common::FindFilesBySuffixRecursively(
      DetermineXmlDir()
        / "Expected"
...
TEST_CASE("Test DescentOnce over an Simple") {
  const std::shared_ptr<
    aas::types::ISimple
  > instance(
    test::common::examples::LoadMaxSimple()
  );
