...
template <
  typename T,
  typename std::enable_if<
    std::is_base_of<T, types::ISimple>::value
  >::type* = nullptr
>
std::pair<
  common::optional<std::shared_ptr<T> >,
...
> ClassFromElement(
  ...
  switch (*model_type) {
    ...
    case types::ModelType::kSimple:
      std::tie(instance, error) = SimpleFromSequence<
        types::IClass
      >(reader);
      break;
...
> SimpleFromElement(
  ...
  if (reader.node().kind() == NodeKind::Error) {
    throw std::logic_error(
      "Unexpected unhandled XML error in SimpleFromElement. "
      "SimpleFromElement expects no reader error at entry."
    );
  }
  #endif
...
enum class OfSimple : std::uint32_t {
  kName = 0
};  // enum class OfSimple
...
> SomethingFromSequence(
  ...
  if (!the_some_items.has_value()) {
    return NoInstanceAndDeserializationErrorWithCause<
      std::shared_ptr<T>
    >(
      L"The required property someItems is missing"
    );
  }
  ...
  if (!the_some_simples.has_value()) {
    return NoInstanceAndDeserializationErrorWithCause<
      std::shared_ptr<T>
    >(
      L"The required property someSimples is missing"
    );
  }
 ...
 * \return error, if any
 */
common::optional<SerializationError> SerializeSimpleAsSequence(
  const types::ISimple& that,
  SelfClosingWriter& writer
);
 ...
 * \return an error, if any
 */
common::optional<SerializationError> SerializeSimpleAsElement(
  const types::ISimple& that,
  SelfClosingWriter& writer
);
...
common::optional<SerializationError> SerializeSimpleAsElement(
  ...
  error = SerializeSimpleAsSequence(
    that,
    writer
  );
  if (error.has_value()) {
