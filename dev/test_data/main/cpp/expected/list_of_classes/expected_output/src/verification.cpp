...
class OfSimple : public impl::IVerificator {
 public:
  OfSimple(
    const std::shared_ptr<types::IClass>& instance
  );
...
bool OfSimple::Done() const {
  return true;
}

const Error& OfSimple::Get() const {
  throw std::logic_error(
    "You want to get from "
    "a verificator OfSimple, "
    "but the verificator is always done as "
...
long OfSimple::Index() const {
  return -1;
}

std::unique_ptr<impl::IVerificator> OfSimple::Clone() const {
  return common::make_unique<
    OfSimple
  >(*this);
}
...
std::unique_ptr<impl::IVerificator> NewNonRecursiveVerificator(
  ...
  switch (instance->model_type()) {
    ...
    case types::ModelType::kSimple:
      return common::make_unique<
        non_recursive_verificator::OfSimple
      >(
        instance
      );
