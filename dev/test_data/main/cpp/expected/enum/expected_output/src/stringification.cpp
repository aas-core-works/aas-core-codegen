...
common::optional<types::Result> ResultFromString(
  const std::string& text
) {
  const auto it = kResultFromStringMap.find(
    text
  );
  if (it == kResultFromStringMap.end()) {
    return {};
  }
  return it->second;
}

types::Result MustResultFromString(
  const std::string& text
) {
  const auto it = kResultFromStringMap.find(
    text
