...
std::pair<
  ...
  >,
  common::optional<DeserializationError>
> DeserializeQueryCondition(
  const nlohmann::json& json,
  bool additional_properties
);

std::set<std::string> kPropertiesInQueryCondition = {
  "$eq",
  "$ne"
};
...
> DeserializeQueryCondition(
  ...
  if (!additional_properties) {
    for (const auto& key_val : json.items()) {
      auto it(
        kPropertiesInQueryCondition.find(key_val.key())
      );
      if (it == kPropertiesInQueryCondition.end()) {
        std::wstring message = common::Concat(
  ...
  // endregion De-serialize $eq

  // region De-serialize $ne
  ...
  switch (that.model_type()) {
    case types::ModelType::kQueryCondition:
      return SerializeQueryCondition(
        dynamic_cast<const types::IQueryCondition&>(that)
      );
    default: {
