...
/**
 * This iterator is always done as IQueryCondition
 * references no other instances.
 */
class IteratorOverQueryCondition : public impl::IIterator {
 public:
  IteratorOverQueryCondition(
  ...
  const std::shared_ptr<types::IClass>& Get() const override {
    throw std::logic_error(
      "You want to get from an IteratorOverQueryCondition, "
      "but the iterator is always done as "
      "IQueryCondition references "
      "no other instances."
    );
  }
  ...
  std::unique_ptr<impl::IIterator> Clone() const override {
    return common::make_unique<IteratorOverQueryCondition>(*this);
  }

  void PrependToPath(Path*) const override {
    throw std::logic_error(
      "You want to prepend to path from an IteratorOverQueryCondition, "
      "but the iterator is always done as "
      "IQueryCondition references "
      "no other instances."
    );
  ...
  ~IteratorOverQueryCondition() override = default;
};  // class IteratorOverQueryCondition
...
std::unique_ptr<impl::IIterator> NewNonRecursiveIterator(
  ...
  switch (instance->model_type()) {
    case types::ModelType::kQueryCondition:
      return common::make_unique<IteratorOverQueryCondition>(
        instance
      );
    default:
      throw std::logic_error(
