...
class OfQueryCondition : public impl::IVerificator {
 public:
  OfQueryCondition(
    const std::shared_ptr<types::IClass>& instance
  );
  ...
  ~OfQueryCondition() override = default;
};  // class OfQueryCondition

OfQueryCondition::OfQueryCondition(
  const std::shared_ptr<types::IClass>&
) {
  // Intentionally empty.
}

void OfQueryCondition::Start() {
  // Intentionally empty.
}

void OfQueryCondition::Next() {
  throw std::logic_error(
    "You want to move "
    "a verificator OfQueryCondition, "
...
bool OfQueryCondition::Done() const {
  return true;
}

const Error& OfQueryCondition::Get() const {
  throw std::logic_error(
...
Error& OfQueryCondition::GetMutable() {
  throw std::logic_error(
...
long OfQueryCondition::Index() const {
  return -1;
}

std::unique_ptr<impl::IVerificator> OfQueryCondition::Clone() const {
...
std::unique_ptr<impl::IVerificator> NewNonRecursiveVerificator(
  ...
  switch (instance->model_type()) {
    case types::ModelType::kQueryCondition:
      return common::make_unique<
        non_recursive_verificator::OfQueryCondition
      >(
        instance
      );
    default:
