...
template <
  ...
  typename std::enable_if<
    std::is_base_of<T, types::IQueryCondition>::value
  >::type* = nullptr
>
std::pair<
...
> kElementNameToModelType = {
  {
    "queryCondition",
    types::ModelType::kQueryCondition
  }
};
...
> ClassFromElement(
  ...
  switch (*model_type) {
    case types::ModelType::kQueryCondition:
      std::tie(instance, error) = QueryConditionFromSequence<
        types::IClass
      >(reader);
      break;
    default:
...
> QueryConditionFromElement(
  ...
  if (reader.node().kind() == NodeKind::Error) {
    throw std::logic_error(
      "Unexpected unhandled XML error in QueryConditionFromElement. "
      "QueryConditionFromElement expects no reader error at entry."
    );
  }
  #endif
...
enum class OfQueryCondition : std::uint32_t {
  kEq = 0,
  kNotEq = 1
};  // enum class OfQueryCondition

const std::unordered_map<
  std::string,
  OfQueryCondition
> kMapOfQueryCondition = {
  {
    "eq",
    OfQueryCondition::kEq
  },
  {
    "not-eq",
    OfQueryCondition::kNotEq
  }
};
...
> QueryConditionFromSequence(
  ...
  if (reader.node().kind() == NodeKind::Error) {
    throw std::logic_error(
      "Unexpected unhandled XML error in QueryConditionFromSequence. "
      "QueryConditionFromSequence expects no reader error at entry."
    );
  }
  #endif
  ...
  common::optional<std::wstring> the_eq;

  common::optional<std::wstring> the_not_eq;
  ...
  while (true) {
    ...
    auto it = properties::kMapOfQueryCondition.find(
      name
    );
    if (it == properties::kMapOfQueryCondition.end()) {
      return NoInstanceAndDeserializationErrorWithCause<
        std::shared_ptr<T>
      >(
        common::Concat(
          L"Expected a start element opening a property "
          L"of IQueryCondition, "
    ...
    const properties::OfQueryCondition property(
      it->second
    );
...
> QueryConditionFrom(
  ...
  std::tie(
    instance,
    error
  ) = QueryConditionFromElement(reader);
 ...
 * \return error, if any
 */
common::optional<SerializationError> SerializeQueryConditionAsSequence(
 ...
 */
common::optional<SerializationError> SerializeQueryConditionAsElement(
  const types::IQueryCondition& that,
  SelfClosingWriter& writer
);

/** @copybrief SerializeQueryConditionAsElement(const types::IQueryCondition&, SelfClosingWriter& */
common::optional<SerializationError> SerializeQueryConditionPtrAsElement(
...
common::optional<SerializationError> SerializeQueryConditionAsElement(
  ...
  error = SerializeQueryConditionAsSequence(
    that,
    writer
  );
  if (error.has_value()) {
...
common::optional<SerializationError> SerializeQueryConditionPtrAsElement(
  ...
  return SerializeQueryConditionAsElement(*that, writer);
}
