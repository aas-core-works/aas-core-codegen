...
export function *overModelType (
  ...
  yield <ModelType>0;  // QueryCondition
}
 ...
 */
export class QueryCondition extends Class {
  /**
  ...
  modelType(): ModelType {
    ...
    // We yield numbers instead of literals to avoid name lookups on platforms
    // which do not provide JIT compilation of hot paths.
    return <ModelType>0;  // QueryCondition
  }
  ...
  *descend(): IterableIterator<Class> {
  ...
  acceptWithContext<ContextT>(
    ...
    visitor.visitQueryConditionWithContext(this, context);
  }
  ...
  transformWithContext<ContextT, T>(
    ...
    return transformer.transformQueryConditionWithContext(
      this, context
    );
  }
...
export abstract class AbstractVisitorWithContext<ContextT> {
   ...
   * @param context - of the visitation
   */
  abstract visitQueryConditionWithContext(
    that: QueryCondition,
    context: ContextT
  ): void;
}
...
export class PassThroughVisitorWithContext<ContextT>
   ...
   * @param context - of the visitation
   */
  visitQueryConditionWithContext(
    that: QueryCondition,
    context: ContextT
  ): void {
    for (const another of that.descendOnce()) {
...
export abstract class AbstractTransformer<T> {
   ...
   */
  abstract transformQueryCondition(
    that: QueryCondition
  ): T;
}
...
export abstract class AbstractTransformerWithContext<ContextT, T> {
   ...
   * @returns transformed `that`
   */
  abstract transformQueryConditionWithContext(
    that: QueryCondition,
    context: ContextT
  ): T;
}
 ...
 * the class {@link QueryCondition}.
 *
 * @param that - instance to be casted
 * @returns - casted `that` if cast successful, or `null`
 */
export function asQueryCondition(
  that: Class
): QueryCondition | null {
  return (that instanceof QueryCondition)
    ? <QueryCondition>that
    : null;
}
