...
test("ReadonlY deserialization fails on non-object", () => {
  const jsonable = "This is not a JSON object.";

  const instanceOrError = AasJsonization.readonlyFromJsonable(
    jsonable
  );
...
test("ReadonlY deserialization fails with missing required property", () => {
...
test("ReadonlY deserialization fails with property type mismatch", () => {
  const jsonable = readExpectedJson(
    "Readonly",
    "minimal.json"
  );
  ...
  const jsonObject = { ...mustBeJsonObject(jsonable) };
  jsonObject["something"] = { definitely: "unexpected-object" };
