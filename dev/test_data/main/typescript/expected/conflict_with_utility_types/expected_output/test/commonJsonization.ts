...
/**
 * Load a maximal example of {@link types.ReadonlY} from
 * the test data directory.
 */
export function loadMaximalReadonly(
): AasTypes.ReadonlY {
  ...
  const casted = AasTypes.asReadonly(instance);
  if (casted === null) {
    throw new Error(
      `Expected instance of ReadonlY in ${aPath}, ` +
...
/**
 * Load a minimal example of {@link types.ReadonlY} from
 * the test data directory.
 */
export function loadMinimalReadonly(
): AasTypes.ReadonlY {
  const aPath = path.join(
    TestCommon.TEST_DATA_DIR,
    "Json",
