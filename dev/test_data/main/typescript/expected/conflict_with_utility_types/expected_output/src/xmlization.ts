...
function parseRecordText(
  ...
  const literal = AasStringification.recordFromString(text);
  if (literal === null) {
    return newDeserializationError<AasTypes.RecorD>(
      `Unexpected literal of RecorD: ${text}`
    );
  }

  return new AasCommon.Either<AasTypes.RecorD, DeserializationError>(
    literal,
    null
  );
}

function serializeRecordText(
  ...
  while (true) {
    ...
    switch (propertyLocalName) {
      case "something": {
        ...
        theSomething = parsedOrError.mustValue();
        break;
      }
  ...
  while (true) {
    ...
    switch (propertyLocalName) {
      ...
      case "aRecord": {
        ...
        theARecord = parsedOrError.mustValue();
        break;
      }
...
/**
  ...
  const serializedAReadonly = this.transform(that.aReadonly);
    parts.push(openTag("aReadonly"));
