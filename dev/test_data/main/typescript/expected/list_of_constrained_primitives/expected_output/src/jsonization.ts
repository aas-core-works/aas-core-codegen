...
class SetterForSomething {
  someNames: Array<string> | null = null;

  /**
   * Parse `jsonable` as the value of {@link someNames}.
   *
   * @param jsonable - to be parsed
...
const SETTER_MAP_FOR_SOMETHING =
  ...
  >(
    [
      [
        "someNames",
        SetterForSomething.prototype.setSomeNamesFromJsonable
      ],
    ]
  );
...
class Serializer extends AasTypes.AbstractTransformer<JsonObject> {
  ...
  transformSomething(
    ...
    const someNamesArray = new Array<string>();
    for (const item of that.someNames) {
      someNamesArray.push(
        item
      );
    }
    jsonable["someNames"] = someNamesArray;
