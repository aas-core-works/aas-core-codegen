...
test("XML namespace mismatch fails", () => {
  const pth = path.join(
    TestCommon.TEST_DATA_DIR, "Xml", "Expected", "someItem", "minimal.xml"
  );
...
test("XML duplicate property fails", () => {
  ...
  if (root.tagName !== "someItem") {
    throw new Error(
      `Expected root element <someItem>, ` +
      `got <${root.tagName}>`
    );
  }
