...
public class Xmlization {
  ...
  private static class _DeserializeImplementation
  {
     ...
     */
    private static _Result<? extends SomeItem> trySomeItemFromElement(
      XMLEventReader reader) {
      return parseInstanceFromElement(
        reader,
        SomeItem.class,
     ...
     */
    private static _Result<? extends AnotherItem> tryAnotherItemFromElement(
      XMLEventReader reader) {
      return parseInstanceFromElement(
        reader,
        AnotherItem.class,
     ...
     */
    private static _Result<? extends Simple> trySimpleFromElement(
      XMLEventReader reader) {
      return parseInstanceFromElement(
        reader,
        Simple.class,
        (elementName, isEmptyElement) -> {
          if (!"simple".equals(elementName)) {
            final Reporting.Error error = new Reporting.Error(
          ...
          return trySimpleFromSequence(reader, isEmptyElement);
        });
    }
      ...
      if (!isEmptySequence) {
        ...
        while (true) {
          ...
          switch (tryElementName.getResult()) {
            case "someItems":
            {
              final _Result<List<IAbstractItem>> trySomeItems = parseList(
                reader,
                isEmptyProperty,
                IAbstractItem.class,
                _DeserializeImplementation::tryIAbstractItemFromElement);

              if (trySomeItems.isError()) {
                trySomeItems.getError()
                  .prependSegment(
                    new Reporting.NameSegment(
                      "someItems"));
                return trySomeItems.castTo(Something.class);
              }

              theSomeItems = trySomeItems.getResult();
              break;
            }
            case "someSimples":
            {
              final _Result<List<ISimple>> trySomeSimples = parseList(
                reader,
                isEmptyProperty,
                ISimple.class,
                _DeserializeImplementation::trySimpleFromElement);

              if (trySomeSimples.isError()) {
                trySomeSimples.getError()
                  .prependSegment(
                    new Reporting.NameSegment(
                      "someSimples"));
                return trySomeSimples.castTo(Something.class);
              }

              theSomeSimples = trySomeSimples.getResult();
              break;
            }
            default:
              final Reporting.Error error = new Reporting.Error(
