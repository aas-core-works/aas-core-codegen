      ...
      if (!isEmptySequence) {
        ...
        while (true) {
          ...
          switch (tryElementName.getResult()) {
            case "someNames":
            {
              final _Result<List<String>> trySomeNames = parseList(
                reader,
                isEmptyProperty,
                String.class,
                _DeserializeImplementation::tryVElementAsString);

              if (trySomeNames.isError()) {
                trySomeNames.getError()
                  .prependSegment(
                    new Reporting.NameSegment(
                      "someNames"));
                return trySomeNames.castTo(Something.class);
              }

              theSomeNames = trySomeNames.getResult();
              break;
            }
            default:
              final Reporting.Error error = new Reporting.Error(
