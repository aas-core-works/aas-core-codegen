      ...
      if (!isEmptySequence) {
        ...
        while (true) {
          ...
          switch (tryElementName.getResult()) {
            case "someResults":
            {
              final _Result<List<Result>> trySomeResults = parseList(
                reader,
                isEmptyProperty,
                Result.class,
                _DeserializeImplementation::tryVElementAsResult);

              if (trySomeResults.isError()) {
                trySomeResults.getError()
                  .prependSegment(
                    new Reporting.NameSegment(
                      "someResults"));
                return trySomeResults.castTo(Something.class);
              }

              theSomeResults = trySomeResults.getResult();
              break;
            }
            default:
              final Reporting.Error error = new Reporting.Error(
