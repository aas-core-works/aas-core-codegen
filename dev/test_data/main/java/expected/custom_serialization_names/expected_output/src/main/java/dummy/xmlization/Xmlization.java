...
public class Xmlization {
  ...
  private static class _DeserializeImplementation
  {
     ...
     */
    private static _Result<QueryCondition> tryQueryConditionFromSequence(
      XMLEventReader reader,
      ...
      if (!isEmptySequence) {
        ...
        while (true) {
          ...
          final _Result<String> tryElementName = tryElementName(reader);
          if (tryElementName.isError()) {
            return tryElementName.castTo(QueryCondition.class);
          }
          ...
          switch (tryElementName.getResult()) {
            case "eq":
            {
              if (isEmptyProperty) {
                ...
                try {
                  theEq = readContentAsString(reader);
                } catch (Exception e) {
                  final Reporting.Error error = new Reporting.Error(
                    "The property eq of an instance of class QueryCondition "
            ...
            case "not-eq":
            {
              if (isEmptyProperty) {
                ...
                try {
                  theNotEq = readContentAsString(reader);
                } catch (Exception e) {
                  final Reporting.Error error = new Reporting.Error(
                    "The property notEq of an instance of class QueryCondition "
                      + " could not be de-serialized: " + e.getMessage());
          ...
          final _Result<XMLEvent> checkEndElement = verifyClosingTagForClass(
            ...
            reader,
            tryElementName);
          if (checkEndElement.isError()) return checkEndElement.castTo(QueryCondition.class);
     ...
     */
    private static _Result<QueryCondition> tryQueryConditionFromElement(
      XMLEventReader reader) {
      skipWhitespaceAndComments(reader);
      ...
      final String elementName = tryElementName.getResult();
      if (!"queryCondition".equals(tryElementName.getResult())) {
        final Reporting.Error error = new Reporting.Error(
      ...
      _Result<QueryCondition> result = tryQueryConditionFromSequence(
        reader,
        isEmptyElement);
      if (result.isError()) return result.castTo(QueryCondition.class);
  ...
  {
    /**
     * Deserialize an instance of QueryCondition from {@code reader}.
     *
     * @param reader Initialized XML reader with reader.peek() set to the element
     */
    public static QueryCondition deserializeQueryCondition(
      XMLEventReader reader) {

      _DeserializeImplementation.skipStartDocument(reader);
      _DeserializeImplementation.skipWhitespaceAndComments(reader);

      _Result<? extends QueryCondition> result =
        _DeserializeImplementation.tryQueryConditionFromElement(
          reader);

      return result.onError(error -> {
        error.prependSegment(new Reporting.NameSegment("querycondition"));
  ...
  static class _VisitorWithWriter
    ...
    @Override
    public void visitQueryCondition(
      IQueryCondition that,
      XMLStreamWriter writer) {
      try {
        writer.writeStartElement(
   ...
   * {@code
   * IClass anInstance = new QueryCondition(
