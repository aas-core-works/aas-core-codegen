...
class _ReaderAndSetterForSomething:
    ...
    def __init__(self) -> None:
        ...
        self.some_names: Optional[List[str]] = None

    def read_and_set_some_names(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        ...
        :py:attr:`.types.Something.some_names` and set it.
        """
...
_READ_AND_SET_DISPATCH_FOR_SOMETHING: Mapping[
    ...
    'someNames':
        _ReaderAndSetterForSomething.read_and_set_some_names,
}
...
class _Serializer(aas_types.AbstractVisitor):
    ...
    def _write_something_as_sequence(
        ...
        """
        if len(that.some_names) == 0:
            self._write_empty_element('someNames')
        else:
            self._write_start_element('someNames')
            for an_item in that.some_names:
                self._write_str_as_element('v', an_item)
            self._write_end_element('someNames')
