...
.. code-block::
    ...
    path = pathlib.Path(...)
    instance = aas_xmlization.read_query_condition_from_file(
        path
    )
...
def query_condition_from_iterparse(
    iterator: Iterator[Tuple[str, Element]]
) -> aas_types.QueryCondition:
    """
    Read an instance of :py:class:`.types.QueryCondition` from
    ...
    .. code-block::
        ...
        with path.open("rt") as fid:
            ...
            instance = aas_xmlization.query_condition_from_iterparse(
                iterator
            )
    ...
    :return:
        Instance of :py:class:`.types.QueryCondition` read from
        :paramref:`iterator`
    """
    ...
    if next_event_element is None:
        raise DeserializationException(
            # fmt: off
            "Expected the start element for QueryCondition, "
    ...
    if next_event != 'start':
        raise DeserializationException(
            f"Expected the start element for QueryCondition, "
            f"but got event {next_event!r} and element {next_element.tag!r}"
        )

    try:
        return _read_query_condition_as_element(
            next_element,
            iterator
        )
...
def query_condition_from_stream(
    stream: TextIO,
    ...
    .. code-block::
        ...
        with open_some_stream_over_network(...) as stream:
            instance = aas_xmlization.query_condition_from_stream(
                stream
            )
    ...
    :param stream:
        ...
        :py:class:`.types.QueryCondition` in XML
    :param has_iterparse:
    ...
    return query_condition_from_iterparse(
        _with_elements_cleared_after_yield(iterator)
    )


def query_condition_from_file(
    path: PathLike,
    has_iterparse: HasIterparse = xml.etree.ElementTree
) -> aas_types.QueryCondition:
    """
    ...
    .. code-block::
        ...
        path = pathlib.Path(...)
        instance = aas_xmlization.query_condition_from_file(
            path
        )
...
def query_condition_from_str(
    ...
    .. code-block::
        ...
        text = "<...>...</...>"
        instance = aas_xmlization.query_condition_from_str(
            text
        )
...
class _ReaderAndSetterForQueryCondition:
    """
    Provide a buffer for reading and setting the properties for the class
    :py:class:`QueryCondition`.

    The properties correspond to the constructor arguments of
    :py:class:`QueryCondition`. We use this buffer to facilitate dispatching when
    ...
    def read_and_set_eq(
        ...
        :py:attr:`.types.QueryCondition.eq` and set it.
        """
        self.eq = _read_str_from_element_text(
            element,
            iterator
        )

    def read_and_set_not_eq(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.QueryCondition.not_eq` and set it.
        """
        self.not_eq = _read_str_from_element_text(
            element,
            iterator
        )


def _read_query_condition_as_sequence(
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
) -> aas_types.QueryCondition:
    """
    Read an instance of :py:class:`.types.QueryCondition`
    ...
    reader_and_setter = (
        _ReaderAndSetterForQueryCondition()
    )
    ...
    while True:
        ...
        read_and_set_method = _READ_AND_SET_DISPATCH_FOR_QUERY_CONDITION.get(
            tag_wo_ns,
            None
        )
    ...
    return aas_types.QueryCondition(
        reader_and_setter.eq,
        reader_and_setter.not_eq
    )


def _read_query_condition_as_element(
    element: Element,
    iterator: Iterator[Tuple[str, Element]]
) -> aas_types.QueryCondition:
    """
    ...
    if tag_wo_ns != 'queryCondition':
        raise DeserializationException(
            f"Expected the element with the tag 'queryCondition', "
            f"but got tag: {tag_wo_ns}"
        )

    return _read_query_condition_as_sequence(
        element,
        iterator
    )
...
#: :py:class:`_ReaderAndSetterForQueryCondition`
_READ_AND_SET_DISPATCH_FOR_QUERY_CONDITION: Mapping[
    str,
    Callable[
        [
            _ReaderAndSetterForQueryCondition,
    ...
    'eq':
        _ReaderAndSetterForQueryCondition.read_and_set_eq,
    'not-eq':
        _ReaderAndSetterForQueryCondition.read_and_set_not_eq,
}
...
_GENERAL_DISPATCH: Mapping[
    ...
    'queryCondition': _read_query_condition_as_sequence,
}
...
class _Serializer(aas_types.AbstractVisitor):
    ...
    def _write_query_condition_as_sequence(
        self,
    ...
    def visit_query_condition(
        ...
        if (
                that.eq is None
                and that.not_eq is None
        ):
            self._write_empty_element(
                'queryCondition'
            )
        else:
            self._write_start_element('queryCondition')
            self._write_query_condition_as_sequence(
                that
            )
            self._write_end_element('queryCondition')
