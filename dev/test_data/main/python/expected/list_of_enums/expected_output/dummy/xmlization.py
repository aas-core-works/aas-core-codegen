...
class _ReaderAndSetterForSomething:
    ...
    def __init__(self) -> None:
        ...
        self.some_results: Optional[List[aas_types.Result]] = None

    def read_and_set_some_results(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        ...
        :py:attr:`.types.Something.some_results` and set it.
        """
        ...
        while True:
            ...
            try:
                item = _read_result_from_element_text(
                    next_element,
                    iterator
                )
            except DeserializationException as exception:
...
def _read_something_as_sequence(
    ...
    if reader_and_setter.some_results is None:
        raise DeserializationException(
            "The required property 'someResults' is missing"
        )

    return aas_types.Something(
        reader_and_setter.some_results
    )
...
_READ_AND_SET_DISPATCH_FOR_SOMETHING: Mapping[
    ...
    'someResults':
        _ReaderAndSetterForSomething.read_and_set_some_results,
}
...
class _Serializer(aas_types.AbstractVisitor):
    ...
    def _write_something_as_sequence(
        ...
        """
        if len(that.some_results) == 0:
            self._write_empty_element('someResults')
        else:
            self._write_start_element('someResults')
            for an_item in that.some_results:
                self._write_str_as_element('v', an_item.value)
            self._write_end_element('someResults')
