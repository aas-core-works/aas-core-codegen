...
class _ReaderAndSetterForSomething:
    ...
    def read_and_set_some_bool(
        ...
        :py:attr:`.types.Something.some_bool` and set it.
        """
        self.some_bool = _read_bool_from_element_text(
            element,
            iterator
        )

    def read_and_set_some_int(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Something.some_int` and set it.
        """
        self.some_int = _read_int_from_element_text(
            element,
            iterator
        )

    def read_and_set_some_float(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Something.some_float` and set it.
        """
        self.some_float = _read_float_from_element_text(
            element,
            iterator
        )

    def read_and_set_some_string(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Something.some_string` and set it.
        """
        self.some_string = _read_str_from_element_text(
            element,
            iterator
        )
    ...
    def read_and_set_some_bytes(
        ...
        """
        self.some_bytes = _read_bytes_from_element_text(
            element,
            iterator
        )
...
def _read_something_as_sequence(
    ...
    if reader_and_setter.some_bool is None:
        raise DeserializationException(
            "The required property 'someBool' is missing"
        )

    if reader_and_setter.some_int is None:
        raise DeserializationException(
            "The required property 'someInt' is missing"
        )

    if reader_and_setter.some_float is None:
        raise DeserializationException(
            "The required property 'someFloat' is missing"
        )

    if reader_and_setter.some_string is None:
        raise DeserializationException(
            "The required property 'someString' is missing"
        )

    if reader_and_setter.some_bytes is None:
        raise DeserializationException(
            "The required property 'someBytes' is missing"
        )

    return aas_types.Something(
        reader_and_setter.some_bool,
        reader_and_setter.some_int,
        reader_and_setter.some_float,
        reader_and_setter.some_string,
        reader_and_setter.some_bytes
    )
...
_READ_AND_SET_DISPATCH_FOR_SOMETHING: Mapping[
    ...
    'someBool':
        _ReaderAndSetterForSomething.read_and_set_some_bool,
    'someInt':
        _ReaderAndSetterForSomething.read_and_set_some_int,
    'someFloat':
        _ReaderAndSetterForSomething.read_and_set_some_float,
    'someString':
        _ReaderAndSetterForSomething.read_and_set_some_string,
    'someBytes':
        _ReaderAndSetterForSomething.read_and_set_some_bytes,
}
