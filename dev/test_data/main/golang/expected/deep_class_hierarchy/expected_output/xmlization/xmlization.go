...
func readNodeWithLookahead(
	...
	switch local {
	case "branch":
		instance, current, err = readBranchAsSequence(
			decoder, current,
		)
	case "leaf":
		instance, current, err = readLeafAsSequence(
			decoder, current,
		)
	case "blossom":
		instance, current, err = readBlossomAsSequence(
			decoder, current,
		)
	default:
		err = newDeserializationError(
...
func readSomethingAsSequence(
	...
	for {
		...
		switch local {
		case "someChoice":
			theSomeChoice, valueErr =  readNodeWithLookahead(
				decoder,
				current,
			)
			// readNodeWithLookahead stops at the end element,
		...
		case "somethingWithoutChoice":
			theSomethingWithoutChoice, valueErr =  readBranchWithLookahead(
				decoder,
				current,
			)
			// readBranchWithLookahead stops at the end element,
			// so we look ahead to the next element, just after the end element.
			if valueErr == nil {
				current, valueErr = readNext(decoder, current)
			}
...
func readContainerAsSequence(
	...
	for {
		...
		switch local {
		case "node":
			theNode, valueErr =  readNodeWithLookahead(
				decoder,
				current,
			)
			// readNodeWithLookahead stops at the end element,
...
// The de-serialization stops by consuming the final end element. The next call to
// the `decoder.Token()` will return the element just after the end element.
func readContainerWithLookahead(
	decoder *xml.Decoder,
	current xml.Token,
) (instance aastypes.IContainer,
	err error,
	...
	instance, current, err = readContainerAsSequence(
		decoder,
		current,
)
	if err != nil {
...
// Serialize the instance of [aastypes.IBranch]
...
// is exactly [aastypes.ModelTypeBranch]. If you need dispatch,
// call [Marshal].
//
...
func writeBranchWithoutDispatch(
	encoder *xml.Encoder,
	that aastypes.IBranch,
	withNamespace bool,
) (err error) {
...
// Serialize the instance of [aastypes.ILeaf]
// enclosed in an XML element which represents the model type.
//
// Do not dispatch on the runtime model type, *i.e.*, assume that the runtime model type
// is exactly [aastypes.ModelTypeLeaf]. If you need dispatch,
// call [Marshal].
//
// If `withNamespace` is set, the `xmlns` attribute is set in the outer XML element.
//
// Flush once the closing end element has been written.
func writeLeafWithoutDispatch(
	encoder *xml.Encoder,
	that aastypes.ILeaf,
	withNamespace bool,
) (err error) {
...
// Serialize the instance of [aastypes.IBlossom]
// enclosed in an XML element which represents the model type.
//
// If `withNamespace` is set, the `xmlns` attribute is set in the outer XML element.
//
...
// Serialize the instance of [aastypes.IContainer]
// enclosed in an XML element which represents the model type.
//
// If `withNamespace` is set, the `xmlns` attribute is set in the outer XML element.
//
...
func Marshal(
	...
	case aastypes.ModelTypeBranch:
		err = writeBranchWithoutDispatch(
			encoder,
			that.(aastypes.IBranch),
			withNamespace,
		)
	case aastypes.ModelTypeLeaf:
		err = writeLeafWithoutDispatch(
			encoder,
			that.(aastypes.ILeaf),
			withNamespace,
		)
