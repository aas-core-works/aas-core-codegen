...
func VerifySomething(
	...
	if that.SomeBools() == nil {
		abort = onError(
			newVerificationError(
				"Required property not set: SomeBools",
			),
		)
		if abort {
			return
	...
	if that.SomeInts() == nil {
		abort = onError(
			newVerificationError(
				"Required property not set: SomeInts",
			),
		)
		if abort {
			return
	...
	if that.SomeFloats() == nil {
		abort = onError(
			newVerificationError(
				"Required property not set: SomeFloats",
			),
		)
		if abort {
			return
	...
	if that.SomeStrings() == nil {
		abort = onError(
			newVerificationError(
				"Required property not set: SomeStrings",
			),
		)
		if abort {
			return
