...
For writing, use the function :py:func:`sdk.xmlization.write` which
...
.. code-block::
    ...
    import sdk.xmlization as aas_xmlization

    path = pathlib.Path(...)
    instance = aas_xmlization.read_my_class_from_file(
        path
    )
...
def my_class_from_iterparse(
    iterator: Iterator[Tuple[str, Element]]
) -> aas_types.MyClass:
    """
    Read an instance of :py:class:`.types.MyClass` from
    ...
    .. code-block::
        ...
        with path.open("rt") as fid:
            ...
            instance = aas_xmlization.my_class_from_iterparse(
                iterator
            )
    ...
    :return:
        Instance of :py:class:`.types.MyClass` read from
        :paramref:`iterator`
    """
    ...
    if next_event_element is None:
        raise DeserializationException(
            # fmt: off
            "Expected the start element for MyClass, "
    ...
    if next_event != 'start':
        raise DeserializationException(
            f"Expected the start element for MyClass, "
            f"but got event {next_event!r} and element {next_element.tag!r}"
        )

    try:
        return _read_my_class_as_element(
            next_element,
            iterator
        )
...
def my_class_from_stream(
    ...
    .. code-block::
        ...
        with open_some_stream_over_network(...) as stream:
            instance = aas_xmlization.my_class_from_stream(
                stream
            )
    ...
    return my_class_from_iterparse(
        _with_elements_cleared_after_yield(iterator)
    )


def my_class_from_file(
    path: PathLike,
    has_iterparse: HasIterparse = xml.etree.ElementTree
) -> aas_types.MyClass:
    """
    ...
    .. code-block::
        ...
        path = pathlib.Path(...)
        instance = aas_xmlization.my_class_from_file(
            path
        )
...
def my_class_from_str(
    ...
    .. code-block::
        ...
        text = "<...>...</...>"
        instance = aas_xmlization.my_class_from_str(
            text
        )
...
def root_from_iterparse(
    iterator: Iterator[Tuple[str, Element]]
) -> aas_types.Root:
    """
    Read an instance of :py:class:`.types.Root` from
    the :paramref:`iterator`.
    ...
    .. code-block::
        ...
        with path.open("rt") as fid:
            ...
            instance = aas_xmlization.root_from_iterparse(
                iterator
            )
    ...
    :raise: :py:class:`DeserializationException` if unexpected input
    :return:
        Instance of :py:class:`.types.Root` read from
        :paramref:`iterator`
    """
    next_event_element = next(iterator, None)
    if next_event_element is None:
        raise DeserializationException(
            # fmt: off
            "Expected the start element for Root, "
            "but got the end-of-input"
            # fmt: on
        )
    ...
    if next_event != 'start':
        raise DeserializationException(
            f"Expected the start element for Root, "
            f"but got event {next_event!r} and element {next_element.tag!r}"
        )

    try:
        return _read_root_as_element(
            next_element,
            iterator
        )
    except DeserializationException as exception:
...
def root_from_stream(
    ...
    .. code-block::
        ...
        with open_some_stream_over_network(...) as stream:
            instance = aas_xmlization.root_from_stream(
                stream
            )
...
def root_from_file(
    ...
    .. code-block::
        ...
        path = pathlib.Path(...)
        instance = aas_xmlization.root_from_file(
            path
        )
...
def root_from_str(
    ...
    .. code-block::
        ...
        text = "<...>...</...>"
        instance = aas_xmlization.root_from_str(
            text
        )
...
class _ReaderAndSetterForMyClass:
    """
    ...
    :py:class:`MyClass`. We use this buffer to facilitate dispatching when
    ...
    def read_and_set_string_prop(
        ...
        :py:attr:`.types.MyClass.string_prop` and set it.
        """
        self.string_prop = _read_str_from_element_text(
            element,
            iterator
        )

    def read_and_set_int_prop(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.MyClass.int_prop` and set it.
        """
        self.int_prop = _read_int_from_element_text(
            element,
            iterator
        )

    def read_and_set_bool_prop(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.MyClass.bool_prop` and set it.
        """
        self.bool_prop = _read_bool_from_element_text(
            element,
            iterator
        )


def _read_my_class_as_sequence(
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
) -> aas_types.MyClass:
    """
    Read an instance of :py:class:`.types.MyClass`
    ...
    while True:
        ...
        read_and_set_method = _READ_AND_SET_DISPATCH_FOR_MY_CLASS.get(
            tag_wo_ns,
            None
        )
    ...
    if reader_and_setter.string_prop is None:
        raise DeserializationException(
            "The required property 'stringProp' is missing"
        )

    if reader_and_setter.int_prop is None:
        raise DeserializationException(
            "The required property 'intProp' is missing"
        )

    if reader_and_setter.bool_prop is None:
        raise DeserializationException(
            "The required property 'boolProp' is missing"
        )

    return aas_types.MyClass(
        reader_and_setter.string_prop,
        reader_and_setter.int_prop,
        reader_and_setter.bool_prop
    )


def _read_my_class_as_element(
    element: Element,
    iterator: Iterator[Tuple[str, Element]]
) -> aas_types.MyClass:
    """
    ...
    if tag_wo_ns != 'myClass':
        raise DeserializationException(
            f"Expected the element with the tag 'myClass', "
            f"but got tag: {tag_wo_ns}"
        )

    return _read_my_class_as_sequence(
        element,
        iterator
    )


class _ReaderAndSetterForRoot:
    """
    Provide a buffer for reading and setting the properties for the class
    :py:class:`Root`.

    The properties correspond to the constructor arguments of
    :py:class:`Root`. We use this buffer to facilitate dispatching when
    parsing the properties in a streaming fashion.
    """
    ...
    def read_and_set_foo(
        ...
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Root.foo` and set it.
        """
        self.foo = _read_my_class_as_sequence(
            element,
            iterator
        )

    def read_and_set_foos(
        self,
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
    ) -> None:
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Root.foos` and set it.
        """
        if element.text is not None and len(element.text.strip()) != 0:
            raise DeserializationException(
        ...
        while True:
            ...
            try:
                item = _read_my_class_as_element(
                    next_element,
                    iterator
                )
            except DeserializationException as exception:
    ...
    def read_and_set_strings(
        ...
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Root.strings` and set it.
        """
        if element.text is not None and len(element.text.strip()) != 0:
            raise DeserializationException(
                f"Expected only item elements and whitespace text, "
    ...
    def read_and_set_integers(
        ...
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Root.integers` and set it.
        """
        if element.text is not None and len(element.text.strip()) != 0:
            raise DeserializationException(
                f"Expected only item elements and whitespace text, "
    ...
    def read_and_set_booleans(
        ...
        """
        Read :paramref:`element` as the property
        :py:attr:`.types.Root.booleans` and set it.
        """
        if element.text is not None and len(element.text.strip()) != 0:
            raise DeserializationException(
                f"Expected only item elements and whitespace text, "
...
def _read_root_as_sequence(
        element: Element,
        iterator: Iterator[Tuple[str, Element]]
) -> aas_types.Root:
    """
    Read an instance of :py:class:`.types.Root`
    as a sequence of XML-encoded properties.
    ...
    while True:
        ...
        read_and_set_method = _READ_AND_SET_DISPATCH_FOR_ROOT.get(
            tag_wo_ns,
            None
        )
        if read_and_set_method is None:
    ...
    if reader_and_setter.foo is None:
        raise DeserializationException(
            "The required property 'foo' is missing"
        )

    if reader_and_setter.foos is None:
        raise DeserializationException(
    ...
    return aas_types.Root(
        reader_and_setter.foo,
        reader_and_setter.foos,
...
def _read_root_as_element(
    ...
    if tag_wo_ns != 'root':
        raise DeserializationException(
            f"Expected the element with the tag 'root', "
            f"but got tag: {tag_wo_ns}"
        )

    return _read_root_as_sequence(
        element,
        iterator
    )
...
#: :py:class:`_ReaderAndSetterForMyClass`
_READ_AND_SET_DISPATCH_FOR_MY_CLASS: Mapping[
    str,
    Callable[
        [
            _ReaderAndSetterForMyClass,
    ...
    'stringProp':
        _ReaderAndSetterForMyClass.read_and_set_string_prop,
    'intProp':
        _ReaderAndSetterForMyClass.read_and_set_int_prop,
    'boolProp':
        _ReaderAndSetterForMyClass.read_and_set_bool_prop,
}


#: Dispatch XML property name to read & set method in
#: :py:class:`_ReaderAndSetterForRoot`
_READ_AND_SET_DISPATCH_FOR_ROOT: Mapping[
    str,
    Callable[
        [
            _ReaderAndSetterForRoot,
    ...
    'foo':
        _ReaderAndSetterForRoot.read_and_set_foo,
    'foos':
        _ReaderAndSetterForRoot.read_and_set_foos,
    'strings':
        _ReaderAndSetterForRoot.read_and_set_strings,
    'integers':
        _ReaderAndSetterForRoot.read_and_set_integers,
    'booleans':
        _ReaderAndSetterForRoot.read_and_set_booleans,
}
...
_GENERAL_DISPATCH: Mapping[
    ...
    'myClass': _read_my_class_as_sequence,
    'root': _read_root_as_sequence,
}
...
class _Serializer(aas_types.AbstractVisitor):
    ...
    def _write_my_class_as_sequence(
        self,
    ...
    def visit_my_class(
        ...
        :param that: instance to be serialized
        """
        self._write_start_element('myClass')
        self._write_my_class_as_sequence(
            that
        )
        self._write_end_element('myClass')

    def _write_root_as_sequence(
        self,
        that: aas_types.Root
    ) -> None:
        """
        ...
        :param that: instance to be serialized
        """
        self._write_start_element('foo')
        self._write_my_class_as_sequence(
            that.foo
        )
        self._write_end_element('foo')

        if len(that.foos) == 0:
            self._write_empty_element('foos')
        else:
            self._write_start_element('foos')
            for an_item in that.foos:
                self.visit(an_item)
            self._write_end_element('foos')

        if len(that.strings) == 0:
            self._write_empty_element('strings')
        else:
            self._write_start_element('strings')
            for another_item in that.strings:
        ...
        if len(that.integers) == 0:
            ...
            for yet_another_item in that.integers:
        ...
        if len(that.booleans) == 0:
            ...
            for yet_yet_another_item in that.booleans:
    ...
    def visit_root(
        ...
        :param that: instance to be serialized
        """
        self._write_start_element('root')
        self._write_root_as_sequence(
            that
        )
        self._write_end_element('root')
