"""Generate code to execute all common XMLization loader functions once."""
...
import * as TestCommonXmlization from "./commonXmlization";"""
    ...
    for concrete_cls in symbol_table.concrete_classes:
        ...
        blocks.append(
            Stripped(
                f"""\
test("commonXmlization loaders for {cls_name_typescript}", () => {{
{I}expect(TestCommonXmlization.{load_minimal_name}()).not.toBeNull();
{I}expect(TestCommonXmlization.{load_maximal_name}()).not.toBeNull();
}});"""
            )
        )
