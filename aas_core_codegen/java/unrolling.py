...
def parentheses_render(node: Node) -> str:
    """
    Render the node recursively with the children enclosed in parentheses.
    ...
    children_renders = map(lambda x: parentheses_render(x), node.children)
    children_render = " ".join(children_renders)
    writer.write(children_render)
