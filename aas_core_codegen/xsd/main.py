        ...
        if isinstance(type_anno.items, intermediate.OurTypeAnnotation):
            ...
            if isinstance(
                ...
                list_element = ET.Element(
                    "xs:choice", {"minOccurs": min_occurs, "maxOccurs": max_occurs}
                )
...
def _define_for_interface(
    ...
    for inheritance in interface.inheritances:
        inheritance_part = ET.Element(
            "xs:group", {"ref": xsd_naming.model_type(inheritance.name)}
        )
    ...
    interface_group = ET.Element(
        "xs:group", {"name": xsd_naming.model_type(interface.name)}
    )
    ...
    if id(interface) in ids_of_used_interfaces:
        ...
        abstract_group = ET.Element(
            "xs:group", {"name": xsd_naming.interface_abstract(interface.name)}
        )
...
def _define_for_class(
    ...
    for interface in cls.interfaces:
        interface_part = ET.Element(
            "xs:group", {"ref": xsd_naming.model_type(interface.name)}
        )
